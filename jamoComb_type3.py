...
for k in range(1,8):
    for j in range(1,10):
        src1 = cv2.imread('./testImg/background2.png') #배경파일 읽기
        for i in range (1,23):
            src2 = cv2.imread('./testImg/jamo/ja'+str(i)+'.png') #초성
            src3 = cv2.imread('./testImg/jamo/mo'+str(j)+'.png') #중성
            src4 = cv2.imread('./testImg/jamo/ja'+str(k)+'.png') #첫번째종성
            ...
            if k == 1 or k == 7: #첫번째 종성이 ㄱ, ㅂ일때 -> 가능한 경우는 ㄳ, ㅄ
                src5 = cv2.imread('./testImg/jamo/ja8.png') #두번째종성 ㅅ
                width, height = src5.shape[:2]
                src1[150:width+150, 140:140+height] = src5
                cv2.imshow('jong2', src1)
                cv2.imwrite('./testImg/jamo/backgroundO/letter'+str(i)+'_'+str(j)+'_'+str(k)+'+8.png', src1)
            elif k == 3: #첫번째 종성이 ㄴ일때 -> 가능한 경우는 ㄵ, ㄶ
                for l in [10,16]:#두번째 종성 ㅈ,ㅎ
                    src5 = cv2.imread('./testImg/jamo/ja'+str(l)+'.png') #두번째종성
                    width, height = src5.shape[:2]
                    src1[150:width+150, 140:140+height] = src5
                    cv2.imshow('jong2', src1)
                    cv2.imwrite('./testImg/jamo/backgroundO/letter'+str(i)+'_'+str(j)+'_'+str(k)+'+'+str(l)+'.png', src1)
            elif k == 5: #첫번째 종성이 ㄹ일때 -> ㄺㄻㄼㄽㄾㄿㅀ
                for l in [1,6,7,8,14,15,16]:#두번째 종성 ㅈ,ㅎ
                    src5 = cv2.imread('./testImg/jamo/ja'+str(l)+'.png') #두번째종성
                    width, height = src5.shape[:2]
                    src1[150:width+150, 140:140+height] = src5
                    cv2.imshow('jong2', src1)
